...
function numberValue(value, fallback = 0.0) {
...
function captainAILena(
    ...
    environment = clamp(
        numberValue(environment)
    );

    scenario = normaliseText(
        scenario,
        "NORMAL"
    );

    sensorIntegrity = normaliseText(
        sensorIntegrity,
        "HIGH"
    );


    const finalOutput = Math.max(
        0.0,
        numberValue(
    ...
    const secondaryOutput = Math.max(
        0.0,
        numberValue(
    ...
    const assessmentDifference = Math.abs(
        primaryOutput -
        secondaryOutput
    );
    ...
    if (assessmentDifference <= 10) {

        assessmentRelationship = "ALIGNED";

    } else if (assessmentDifference <= 25) {

        assessmentRelationship = "DIVERGENT";

    } else {

        assessmentRelationship = "SIGNIFICANT_DIVERGENCE";
    }
    ...
    const stabilizerSource = normaliseText(
    ...
    /*
     * Captain AI Lena performs autonomous assessment
     ...
     *
     * Only the separate execution gate may permit
     * an action within the defined operating envelope.
     */

    const autonomousDecision = true;

    const trialManoeuvreRequired = true;

    const validationRequired = true;

    const directActuatorCommand = false;
